...
from scripts.yahoo_history import _batch_frame, completed_sessions, validated_window
...
def _metrics(frame: pd.DataFrame, spy: pd.DataFrame, ticker: str, date: str) -> dict:
    ...
    for length in (1, 5, 20):
        current = float(closes.iloc[-1]); start = float(closes.iloc[-1 - length])
        ...
        returns[str(length)] = round(ret, 2)
        relative[str(length)] = round(ret - baseline, 2)
    ...
    return {
        ...
        "volume": int(latest_volume), "rvol": round(latest_volume / mean, 2),
...
def build_theme_snapshot(core: pd.DataFrame, market_date: str,
    ...
    for ticker in [t for info in THEMES.values() for t in info["etfs"] + info["stocks"]]:
        ...
        try:
            if ticker in core.columns.get_level_values(0):
                frame = validated_window(core[ticker], ticker, dates)
            else:
                try:
                    frame = validated_window(_batch_frame(batch, ticker), ticker, dates)
                except Exception:
                    history = yf.Ticker(ticker).history(raise_errors=True, **params)
                    frame = validated_window(history, ticker, dates)
    ...
    with requests.Session() as session:
        ...
        for ticker, url in ISSUER_FUNDS.items():
            records = _verified_past(past.get(ticker), ticker, dates)
            try:
                response = session.get(url, timeout=(5, 15))
                response.raise_for_status()
                record = parse_issuer_html(response.text, ticker)
                if record["date"] != market_date:
                    raise ValueError(f"issuer date {record['date']} differs from {market_date}")
            ...
            flows[ticker] = flow_windows(issuer[ticker], dates)
    return {
